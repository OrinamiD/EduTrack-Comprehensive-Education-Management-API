...
import { agriculture, basicScience, basicTech, biology, chemistry, commerce, CRS, economics, english, furtherMathematics, goverment, mathematics, physics, socialStudies, } from "../utils/subject.code.utils.js";
// create subject
export const subjectCreation = async (data) => {
    const { schoolId, userId, classId, code } = data;
    const existingSchool = await School.findById(schoolId);
    if (!existingSchool) {
        throw new Error("you are not recognize in this college. Please pick your form today");
    }
...
// update subject
export const subjectUpdate = async (data) => {
    ...
    const update = await Subject.findByIdAndUpdate(id, { name, code, description }, { new: true });
    await update?.save();
...
export const allSubject = async (userId) => {
    const user = await User.findById(userId);
    if (!user) {
        throw new Error("User does not exist");
    }
...
// delete subject
export const subjectDeletion = async (subjectId, userId) => {
    const user = await User.findById(userId);
    if (!user) {
        throw new Error("user does not exist");
    }
